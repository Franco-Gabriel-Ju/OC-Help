...
class CPU_UI:
    ...
    def crear_barra_inferior(self):
        ...
        estado_lbl = ttk.Label(bottom, textvariable=self.estado_var, anchor="w",
                               foreground="gray", font=("Courier", 9))
        estado_lbl.grid(row=2, column=0, columnspan=2, sticky="ew", padx=4)

        self.instruccion_var = tk.StringVar(value="")
        instruccion_lbl = ttk.Label(bottom, textvariable=self.instruccion_var, anchor="w",
                                    font=("Courier", 12, "bold"), foreground="blue")
        instruccion_lbl.grid(row=3, column=0, columnspan=2, sticky="ew", padx=4, pady=(2, 4))
        ...
        for widget in self.main.winfo_children():
            if isinstance(widget, ttk.Frame):
                for child in widget.winfo_children():
                    if isinstance(child, ttk.Label) and child.cget("textvariable") == str(self.estado_var):
