...
def sumar_nc(
    ...
    """Suma dos valores en sistema NC fijo.

    Flujo:
    1) Descomplementa ambos operandos.
    2) Suma los valores explícitos.
    3) Reaplica complemento NC al resultado.

    Retorna una tupla: (resultado_nc, valor_explicito_suma)
    """
    ...
    # Descomplementar ambos operandos
    valor1_explicito = descomplementar(
        valor1_complementado,
        base_nc,
        separador=separador,
    ...
    valor2_explicito = descomplementar(
        valor2_complementado,
        base_nc,
        separador=separador,
        enteros_totales=enteros_totales,
    ...
    decimal1 = _a_decimal(valor1_explicito, base_nc)
    decimal2 = _a_decimal(valor2_explicito, base_nc)

    # Suma
    suma_decimal = decimal1 + decimal2

    # Convertir resultado de vuelta a string con signo
    if suma_decimal < 0:
        signo_suma = "-"
        suma_decimal_abs = abs(suma_decimal)
    else:
        signo_suma = ""
        suma_decimal_abs = suma_decimal

    entero_str, fraccion_str = _redondear_absoluto_en_base(
        suma_decimal_abs, base_nc, fracc_totales
    )

    valor_suma_explicito = f"{signo_suma}{entero_str}"
    if fraccion_str:
        valor_suma_explicito += f"{separador}{fraccion_str}"

    # Reaplicar complemento al resultado
    if suma_decimal < 0:
        # Aplicar complemento
        if len(entero_str) > enteros_totales:
            raise ConversionError(
                f"El resultado no entra en {enteros_totales} enteros tras la suma."
            )
        entero_str = entero_str.zfill(enteros_totales)

        resultado_nc = _complemento_base_hibrido(
            entero_str, fraccion_str, base_nc, "complemento"
        )
    else:
        # Sin complemento, solo rellenar
        if len(entero_str) > enteros_totales:
            raise ConversionError(
                f"El resultado no entra en {enteros_totales} enteros tras la suma."
            )
        entero_str = entero_str.zfill(enteros_totales)
        resultado_nc = f"{entero_str}"
        if fraccion_str:
            resultado_nc += f"{separador}{fraccion_str}"

    return resultado_nc.replace(".", separador), valor_suma_explicito
