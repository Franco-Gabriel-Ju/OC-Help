...
class CalculadoraConversorApp:
    ...
    def _construir_ui(self):
        ...
        ttk.Label(entrada_frame, text="Separador decimal:").grid(row=2, column=2, sticky="w", pady=4, padx=(10, 0))
        ...
        sep_combo.grid(row=2, column=3, sticky="w", padx=(10, 0), pady=4)

        opt_frame = ttk.LabelFrame(main, text="Opciones avanzadas", padding=10)
        opt_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=(10, 0))
        opt_frame.columnconfigure(1, weight=1)
        opt_frame.columnconfigure(3, weight=1)

        ttk.Label(opt_frame, text="Complemento:").grid(row=0, column=0, sticky="w", pady=4)
        ...
        comp_combo.grid(row=0, column=1, sticky="w", padx=(10, 0), pady=4)
        
        # Nota informativa sobre complemento
        info_label = ttk.Label(opt_frame, text="(En bases no-binarias, se aplica en binario)", foreground="gray")
        info_label.grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 4))

        ttk.Label(opt_frame, text="Bits (complemento):").grid(row=0, column=2, sticky="w", pady=4, padx=(10, 0))
        bits_spin = ttk.Spinbox(
            opt_frame,
            from_=8,
            to=64,
        ...
        bits_spin.grid(row=0, column=3, sticky="w", padx=(10, 0), pady=4)

        botones = ttk.Frame(main)
        botones.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(10, 8))
        ...
        salida_frame.grid(row=4, column=0, columnspan=2, sticky="nsew", pady=(10, 0))
        ...
        ttk.Label(
            salida_frame,
            text="Tip: usa letras A-Z para bases mayores a 10. Complemento solo aplica a binario.",
            foreground="#666666",
            font=("Segoe UI", 9),
        ).grid(row=1, column=0, sticky="w", pady=(8, 0))

        estado = ttk.Label(main, textvariable=self.estado_var, foreground="#666666")
        estado.grid(row=5, column=0, columnspan=2, sticky="w", pady=(8, 0))
