...
function initTabs() {
  ...
  const optionsToggle = byId("calc-options-toggle");
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      if (options && optionsToggle && mobileQuery.matches) {
        options.classList.remove("open");
        optionsToggle.classList.remove("is-open");
...
function initCalcOptionsDrawer() {
  const options = byId("calc-options");
  const toggle = byId("calc-options-toggle");
  if (!options || !toggle) {
    return;
  }
