...
function clearCalcOutputs() {
  ...
  byId("calc-manual-out").textContent = "Pulsa \"Detalle\" en cualquier sistema para ver la resolución paso a paso.";
}
...
async function loadManualDetail(target) {
  ...
  if (data.ok) {
    byId("calc-manual-out").classList.remove("error");
    byId("calc-manual-out").textContent = data.detalle;
    return;
  }

  byId("calc-manual-out").classList.add("error");
  byId("calc-manual-out").textContent = data.error || "No se pudo generar el detalle manual.";
}
