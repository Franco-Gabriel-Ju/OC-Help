...
def convertir(numero, base_origen, base_destino, precision=12, complemento=None, bits_complemento=None, separador="."):
    ...
    Puede aplicar complemento por mascara hibrida en la base destino:
    - a_uno: complemento a la base disminuida (C_{n-1})
    - a_dos: complemento a la base (C_n)
    """
    _validar_base(base_origen)
    ...
    if complemento and signo == "-":
        if complemento not in ("a_uno", "a_dos", "base"):
            raise ConversionError("Complemento invalido. Use 'a_uno' o 'a_dos'.")

        # Compatibilidad: en binario, bits_complemento puede forzar ancho entero minimo.
        if base_destino == 2 and bits_complemento:
            if not isinstance(bits_complemento, int) or bits_complemento < 1:
                raise ConversionError("bits_complemento debe ser un entero positivo.")
            entero_str = entero_str.zfill(bits_complemento)
