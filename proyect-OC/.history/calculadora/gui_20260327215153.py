...
class CalculadoraConversorApp:
    ...
    def _convertir_todo(self):
        ...
        if self.entrada_complementada_var.get():
            numero = descomplementar(numero, base_origen, separador=separador)
            complemento = None
        ...
        try:
            ...
            for key, base_dest in self.BASES.items():
                if self.redondear_var.get():
                    precision_objetivo = max(precision, self._precision_minima_exacta(numero, base_origen, base_dest))
                else:
